...
create_mapping_sub(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, uint64 perm) {
    /*
    pgtbl 为根页表的基地址
    va, pa 为需要映射的虚拟地址、物理地址
    sz 为映射的大小，单位为字节
   ...
   while(va < end)
   {
     ...
     tbl1 = (uint64* )((((pte2) >> 10) << 12) + PA2VA_OFFSET);
     vpn1 = (va >> 21) & 0x1ff;
     pte1 = tbl1[vpn1];         
     if((pte1 & 0x1) == 0){
        uint64* k = (uint64* )kalloc();
     ...
     tbl0 = (uint64* )(((pte1 >> 10) << 12) + PA2VA_OFFSET);
     vpn0 = (va >> 12) & 0x1ff;
     pte0 = ((pa >> 12) << 10) | perm;
     tbl0[vpn0] = pte0; 
     va += PGSIZE;
